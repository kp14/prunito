...
from .pointcloud import (
    get_points_equiangularly_distanced_on_sphere,
...
class SwissProtRecordCollector(object):
    ...
    def collect_record(self, sp_record):
        ...
        self.graph.add_node(
            self.current_entry, {"class": "ac", "typ": None, "group": 1}
        )
            ...
            for comment_type in CommentTypes.InterestingTypes:
                if comment.startswith(comment_type):
                    ...
                    for specific_string in type_specific_string_list:
                        ...
                        if cleaned_string:
                            self.add_or_count_key(
                                cleaned_string, "comment", typ=self.context, group=2
                            )
            ...
            if prefix in ["RecName: Full", "AltName: Full", "EC"]:
                ...
                if len(words_in_name) > 1:
                    for word in words_in_name:
                        self.add_or_count_key(
                            word.lower(), "name", typ=self.context, group=2
                        )
    ...
    def __collect_features(self, list_of_tuples):  # TODO: Have a whitelist or blacklist
        ...
        for item in list_of_tuples:
            f = Feature(
                key=item[0],
                description=SPQualifiers.remove_qualifiers(item[3]).rstrip(" ."),
            )
            self.add_or_count_key(
                "{0}-{1}".format(f.key, f.description), "feature", typ=f.key, group=6
            )
    ...
    def add_or_count_key(self, a_key, _class, typ=None, group=1):
        if self.graph.has_node(a_key):
            ...
            self.graph.add_node(
                a_key, {"freq": 1, "class": _class, "typ": typ, "group": group}
            )
    ...
    def summarize_all(self, cutoff=0.0):
        ...
        try:
            data_nodes = [
                n for n in self.graph.nodes() if not self.graph.node[n]["class"] == "ac"
            ]
        except KeyError:
    ...
    def _get_neighbors_by_class(self, node_of_interest, class_):
        return [
            n
            for n in self.graph[node_of_interest].keys()
