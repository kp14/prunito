...
def test_tax_node_info_one_id():
    ...
    actual = up.tax_node_info('9606')
    assert len(expected['taxonomies']) == len(actual['taxonomies'])
    assert len(actual['taxonomies']) == 1
    for k, v in expected['taxonomies'][0].items():
        assert actual['taxonomies'][0][k] == v


def test_tax_node_info_several_ids():
    expected = {
              "taxonomies": [
                {
                  "taxonomyId": 9606,
