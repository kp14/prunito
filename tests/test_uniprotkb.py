...
def test_object_is_record_instance():
    assert isinstance(my_record[0],
                      up.parsers.parser_knowledgebase_txt.Record) == True
...
def test_description():
    assert my_record[
        0].description == "RecName: Full=Nucleotide-binding oligomerization domain-containing protein 2; EC=2.3.1.234 {ECO:0000255|HAMAP-Rule:MF_01445, ECO:0000269|PubMed:22378793}; AltName: Full=Caspase recruitment domain-containing protein 15; EC=6.3.1.999;"


def test_recommended_full_name():
    assert my_record[
        0].recommended_full_name == 'Nucleotide-binding oligomerization domain-containing protein 2'
...
def test_gene_name():
    assert my_record[
        0].gene_name == 'Name=NOD2 {ECO:0000255|HAMAP-Rule:MF_01445}; Synonyms=CARD15;'
...
def test_organism_classification():
    assert my_record[0].organism_classification == [
...
def test_reference_positions():
    assert my_record[0].references[0].positions == [
        'NUCLEOTIDE SEQUENCE [GENOMIC DNA / MRNA], AND VARIANTS ALA-70;',
...
def test_reference_references():
    assert my_record[0].references[0].references == [
        ('PubMed', '16897345'), ('DOI', '10.1007/s00335-005-0148-2')
    ]
...
def test_reference_authors():
    assert my_record[0].references[
        0].authors == 'Taylor K.H., Taylor J.F., White S.N., Womack J.E.'
...
def test_reference_title():
    assert my_record[0].references[
        0].title == 'Identification of genetic variation and putative regulatory regions in bovine CARD15.'


def test_reference_location():
    assert my_record[0].references[
        0].location == 'Mamm. Genome 17:892-901(2006).'


def test_list_of_pmids():
    assert my_record[0].all_pubmed_ids == [
        '16897345', '20698950', '16203728', '18511561', '19592251', '21887730'
    ]
...
def test_comments_content():
    assert my_record[0].comments[
        6] == 'SIMILARITY: Contains 2 CARD domains. {ECO:0000255|PROSITE-ProRule:PRU00046}.'


def test_cross_references():
    assert my_record[0].cross_references[0] == ('EMBL', 'AY518737',
                                                'AAS09824.1', '-', 'mRNA')
...
def test_organism_classification_compatibility():
    assert biopython_record.organism_classification == my_record[
        0].organism_classification
...
def test_ref_author_compatibility():
    assert my_record[0].references[0].authors == biopython_record.references[
        0].authors
